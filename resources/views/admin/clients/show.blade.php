...
<div class="bg-white rounded-lg shadow p-6">
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <p class="text-lg font-semibold">{{ $client->adresse }}</p>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-500">Nationalité</label>
            <p class="text-lg font-semibold">{{ $client->nationalite }}</p>
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-500">Genre</label>
            <p class="text-lg font-semibold">{{ $client->genre }}</p>
        </div>
        <div>
    ...
    <div class="flex justify-end space-x-3 mt-6">
        <a href="{{ route('admin.clients.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">Retour</a>
    </div>
</div>
@endsection