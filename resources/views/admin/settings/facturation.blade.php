...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6">
    <div class="bg-white rounded-lg shadow p-4">
        ...
        <ul class="space-y-2">
            ...
            <li><a href="{{ route('admin.settings.facturation') }}" class="block px-3 py-2 rounded bg-blue-50 text-blue-600 font-semibold">💰 Facturation</a></li>
    ...
    <div class="md:col-span-3 bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('admin.settings.update.facturation') }}">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">TVA (%)</label>
                <input type="number" step="0.1" name="tva" value="{{ config('billing.tva') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Frais de service</label>
                <input type="number" step="100" name="service_fee" value="{{ config('billing.service_fee') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
