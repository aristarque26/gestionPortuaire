...
@section('title', 'Paramètres - Apparence')
@section('header', 'Apparence')
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6">
    <div class="bg-white rounded-lg shadow p-4">
        ...
        <ul class="space-y-2">
            ...
            <li><a href="{{ route('admin.settings.apparence') }}" class="block px-3 py-2 rounded bg-blue-50 text-blue-600 font-semibold">🎨 Apparence</a></li>
    ...
    <div class="md:col-span-3 bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('admin.settings.update.apparence') }}">
            @csrf
            <div class="mb-4">
                ...
                <select name="theme" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                    <option value="light" {{ config('settings.theme') == 'light' ? 'selected' : '' }}>Clair</option>
                    <option value="dark" {{ config('settings.theme') == 'dark' ? 'selected' : '' }}>Sombre</option>
                </select>
            </div>
