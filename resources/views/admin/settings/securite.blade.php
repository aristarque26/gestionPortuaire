...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6">
    <div class="bg-white rounded-lg shadow p-4">
        ...
        <ul class="space-y-2">
            ...
            <li><a href="{{ route('admin.settings.securite') }}" class="block px-3 py-2 rounded bg-blue-50 text-blue-600 font-semibold">🔒 Sécurité</a></li>
    ...
    <div class="md:col-span-3 bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('admin.settings.update.securite') }}">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Session timeout (minutes)</label>
                <input type="number" name="session_timeout" value="{{ config('session.lifetime') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">2FA (double authentification)</label>
                <select name="two_factor" class="w-full border border-gray-300 rounded-lg px-3 py-2">
