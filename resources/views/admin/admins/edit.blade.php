...
@push('styles')
    ...
    <style>
        /* (Les mêmes styles que la création, on pourrait les mutualiser) */
        body { font-family: 'Inter', sans-serif; background: linear-gradient(135deg, #f0f4ff 0%, #e6edf7 100%); min-height: 100vh; display: flex; align-items: center; justify-content: center; }
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow-xl p-8 card-animate">
        ...
        <form method="POST" action="{{ route('admin.admins.update', $admin->id) }}" autocomplete="off" id="editForm">
            @csrf
            @method('PUT')
            ...
            {{-- Indicateur de force (pour le nouveau mot de passe) --}}
            ...
            {{-- Info : laisser vide pour conserver --}}
            <div class="text-xs text-gray-500 mb-4" style="margin-left: 2.8rem; margin-top: -0.5rem;">
                <i class="fas fa-info-circle mr-1"></i> Laissez vide pour conserver le mot de passe actuel.
            </div>

            {{-- Statut (toggle) --}}
            <div class="toggle-switch">
                <input type="checkbox" id="statutToggle" name="statut" value="actif" {{ $admin->statut == 'actif' ? 'checked' : '' }}>
                <label for="statutToggle" class="slider"></label>
                <span class="label-text">Statut : <span id="statutLabel">{{ $admin->statut == 'actif' ? 'Actif' : 'Inactif' }}</span></span>
            </div>
            <input type="hidden" name="statut" id="statutHidden" value="{{ $admin->statut }}">
            ...
            <div class="flex items-center justify-end gap-4 mt-6">
                ...
                <button type="submit" class="btn-ripple bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-3 rounded-xl shadow-lg transition-all duration-300 flex items-center gap-2" id="submitBtn">
                    <span id="btnText"><i class="fas fa-sync-alt"></i> Mettre à jour</span>
                    <span id="btnSpinner" class="hidden"><i class="fas fa-spinner fa-spin"></i> Mise à jour...</span>
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    // Force du mot de passe (si champ rempli)
    ...
    form.addEventListener('submit', function(e) {
        // Si le champ password est vide, on ne vérifie pas la confirmation
        if (passwordInput.value.length > 0 && confirmInput.value.length > 0 && passwordInput.value !== confirmInput.value) {
            e.preventDefault();
            confirmError.textContent = 'Les mots de passe ne correspondent pas.';
            confirmInput.classList.add('is-invalid');
            return;
